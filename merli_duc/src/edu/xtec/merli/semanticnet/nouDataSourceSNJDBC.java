...
//			 TODO Auto-generated method stub
		...
		/**
		 * Retorna el llistat de nodes amb l'identificador a 'lnod' i del t�pus 'nodeType'.
		 * @param lnod Llistat de identificador dels nodes que es vol carregar.
		 * @param nodeType T�pus del nodes que es volen carregar.
		 * @return {nodeDTO} on 'nodesDTO' �s un hashtable amb la informaci� necessaria
		...
		public List getNodes(List lnod, String nodeType) throws SemanticException {
			...
			if (!lnod.isEmpty()){
				...
				/*Recupera tots els nodes del t�pus donat.*/
				condicio = " ";
			}
		 ...
		 * del node 'idnode' del t�pus 'nodeType'
		 * @param idNode Identificador del node.
		 * @param nodeType T�pus del node 'idNode'.
		 * @param type T�pus de la relaci�.
		 * @param direction Direcci� de les relacions buscades.
		 * @return Retorna un llistat {dtonode} on 'dtonode' �s un hashtable amb la informaci� 
		 * del node. 
		 * @throws SemanticException
		 */
		public List getRelations(int idNode, String nodeType , String type, int direction) throws SemanticException {
			...
			try {
				...
				for (int i=0;i< l1.size();i++){
					/*En cas que algun dels camps sigui 'null' es creu que la relaci� no existeix.
					 * 
					 */
					try{
						Hashtable dto = new Hashtable();
		...
		private ConnectionBean connectBD() throws SemanticException{
			...
			try{
				...
				the.setStackTrace(e.getStackTrace());
				throw the;
				}//throw new Exception("Error en la connexi� amb la base de dades.");}		
		...
		/**
		 * converteix un dto en un String per executar una operaci� de 'insert' o de 'update'
		 ...
		 * @return Caden a punt per formar part d'una instrucci� SQL de 'insert' o de 'update'
		 * @throws Exception
		 */
		...
		public int addNewNode(Hashtable DTO) throws SemanticException {
			...
			try{
				cb = this.connectBD();
				//Recupera el seg�ent valor d'identificador per insertar a la taula.
...
//				Inserta les noves relacions amb thesaurus keys.
		...
		/**
		 * Retorna un llistat amb tots els atributs d'un node del t�pus 'nodeType'.
		 * @param nodeType T�pus de node.
		 * @param lorder Parametre de sortida. Cont� els camps per realitzar una consulta
		 * ordenada
		...
		/**
		 * Converteix un map a dto representatiu d'un node del t�pus 'nodeType'.
		 * @param m Map resultant d'una consulta SQL al sistema.
		 * @param nodeType T�pus de nodes esperats.
		 ...
		 * @param nodeType T�pus del node que es vol consultar.
		 * @param type T�pus de relaci� que es busca.
		 * @param direction Direcci� de la relaci�.
